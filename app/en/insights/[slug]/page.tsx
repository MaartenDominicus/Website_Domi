...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${original.title} | Domi Installation`,
    description: article.text,
    alternates: { canonical: `/en/insights/${article.slug}`, languages: { nl: `/kennis/${article.slug}`, en: `/en/insights/${article.slug}` } },
    openGraph: { title: original.title, description: article.text, type: "article", locale: "en_GB", images: [{ url: article.image, alt: article.alt }] },
